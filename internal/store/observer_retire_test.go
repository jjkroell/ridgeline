...
func TestUpdateObserverStatusIfPresentNeverCreates(t *testing.T) {
	...
	found, err := st.UpdateObserverStatusIfPresent("ghost-obs", "YCD", "AA", `{"state":"online"}`, "910.5,250,11,5", now)
	if err != nil {
	...
	if err := st.UpsertObserverStatus("real-obs", "YCD", "BB", `{"state":"online"}`, "910.5,250,11,5", now); err != nil {
		t.Fatalf("UpsertObserverStatus: %v", err)
	}
	found, err = st.UpdateObserverStatusIfPresent("real-obs", "YCD", "BB", `{"state":"offline"}`, "", now)
	if err != nil || !found {
