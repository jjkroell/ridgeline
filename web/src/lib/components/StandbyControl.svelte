...
<script lang="ts">
	...
	// `block` wraps the button in its own spaced row (the mobile layout wants
	// that). It lives INSIDE the admin gate on purpose: a wrapper supplied by the
	// caller renders for everyone and leaves an empty, margin-bearing div on the
	// page for non-admins.
	let {
		observer,
		id,
